...
> ⚠️ **重要安全提示**
>
> 书源规则中的 `@js:` 表达式、`jsLib` 全局库、`{{@js: ...}}` 内联计算均会在运行时执行任意 JavaScript 代码。这是设计预期行为，用于支持复杂的页面解析和数据处理需求。
>
> **安全建议**：
> - **仅导入可信来源的书源**：书源文件等同于可执行代码，请勿导入来历不明的书源
> - **审查书源内容**：导入前建议检查书源中的 `jsLib` 和 `@js:` 规则内容
> - **自行承担风险**：导入第三方书源产生的安全问题由用户自行负责
>
> 此设计与 legado、Tachiyomi/Mihon 等主流阅读器一致，是实现灵活书源解析的必要能力。
